const links = [
  // {
  //   category: 'Ongoing',
  //   links: [
  //     {
  //       name: 'work-nav-links',
  //       url: 'https://work.zootechs.com/',
  //     },
  //     {
  //       name: '2023-tech-schedule',
  //       url: 'https://dida365.com/webapp/#p/5fe2df9586da912052ea9847/tasks/63df68cbaaaef0270e1e07c0',
  //     },
  //     {
  //       name: 'notion-daily',
  //       url: 'https://www.notion.so/daily-8813d1c8a0e04085bc51ed3205e5f6fc',
  //     },
  //     {
  //       name: '滴答清单-work',
  //       url: 'https://dida365.com/webapp/#p/e1134c90b27575dc7f84ab0e/tasks',
  //     },
  //   ],
  // },
  {
    category: 'News',
    links: [
      {
        name: 'Financial Times',
        url: 'https://www.ft.com/',
      },
      {
        name: 'The Wall Street Journal',
        url: 'https://www.wsj.com/',
      },
      {
        name: '华尔街日报',
        url: 'https://cn.wsj.com/',
      },
      {
        name: '南方周末',
        url: 'http://www.infzm.com/',
      },
      {
        name: '界面新闻',
        url: 'https://www.jiemian.com/',
      },
      {
        name: 'The New York Times',
        url: 'https://www.nytimes.com/',
      },
      {
        name: 'FT中文网',
        url: 'http://www.ftchinese.com/',
      },
      {
        name: '财新网',
        url: 'https://www.caixin.com/',
      },
      {
        name: 'WIRED',
        url: 'https://www.wired.com/',
      },
      {
        name: 'South China Morning Post',
        url: 'https://www.scmp.com/',
      },
      {
        name: 'The Economist',
        url: 'https://www.economist.com/',
      },
      {
        name: 'The Atlantic',
        url: 'https://www.theatlantic.com/world/',
      },
      {
        name: 'The Guardian',
        url: 'https://www.theguardian.com/international',
      },
      {
        name: 'Bloomberg',
        url: 'https://www.bloomberg.com/',
      },
      {
        name: 'HuffPost',
        url: 'https://www.huffpost.com/',
      },
      {
        name: 'MIT News',
        url: 'https://news.mit.edu/',
      },
      {
        name: 'Fortune',
        url: 'http://www.fortunechina.com/',
      },
    ],
  },
  {
    category: 'IT Tech',
    links: [
      {
        name: 'Hacker news',
        url: 'https://news.ycombinator.com/news',
      },
      {
        name: 'GitHub',
        url: 'http://github.com/',
      },
      {
        name: 'InfoQ',
        url: 'https://www.infoq.com/',
      },
      {
        name: 'The New Stack',
        url: 'https://thenewstack.io/',
      },
      {
        name: 'Changelog',
        url: 'https://changelog.com/',
      },
      {
        name: 'v2ex',
        url: 'https://v2ex.com/',
      },
      {
        name: 'SegmentFault',
        url: 'https://segmentfault.com/',
      },
      {
        name: 'DEV Community 👩‍💻👨‍💻',
        url: 'https://dev.to/',
      },
      {
        name: 'CSS-Tricks',
        url: 'https://css-tricks.com/',
      },
      {
        name: 'TutorialEdge.net',
        url: 'https://tutorialedge.net/',
      },
      {
        name: 'stackoverflow blog',
        url: 'https://stackoverflow.blog/',
      },
      {
        name: 'Derick Rethans',
        url: 'https://derickrethans.nl/',
      },
      {
        name: 'Go Blog',
        url: 'https://blog.golang.org/index',
      },
      {
        name: 'Golang By Example',
        url: 'https://golangbyexample.com/',
      },
      {
        name: 'Taobao MySQL',
        url: 'http://mysql.taobao.org/monthly/',
      },
      {
        name: 'CSS Inspiration',
        url: 'https://chokcoco.github.io/CSS-Inspiration',
      },
      {
        name: 'Modern JavaScript',
        url: 'https://javascript.info/',
      },
      {
        name: 'Go Questions',
        url: 'https://www.bookstack.cn/read/qcrao-Go-Questions/README.md',
      },
      {
        name: 'CSS reference',
        url: 'https://cssreference.io/',
      },
      {
        name: 'Flavio Copes',
        url: 'https://flaviocopes.com/',
      },
      {
        name: 'devconnected',
        url: 'https://devconnected.com/',
      },
      {
        name: 'HTML reference',
        url: 'https://htmlreference.io/',
      },
      {
        name: 'CSS Layout',
        url: 'https://csslayout.io/',
      },
      {
        name: 'Morden CSS',
        url: 'https://moderncss.dev/',
      },
      {
        name: 'rapid7',
        url: 'https://blog.rapid7.com/',
      },
      {
        name: 'studygolang',
        url: 'https://studygolang.com/',
      },
      {
        name: 'Go Web Examples',
        url: 'https://gowebexamples.com/',
      },
      {
        name: 'Gophercises',
        url: 'https://gophercises.com/',
      },
      {
        name: 'courses calhoun',
        url: 'https://courses.calhoun.io/courses',
      },
      {
        name: 'Kali Docs',
        url: 'https://www.kali.org/docs/',
      },
      {
        name: 'Laracasts',
        url: 'https://laracasts.com/',
      },
      {
        name: 'QuickRef.ME',
        url: 'https://quickref.me/',
      },
      {
        name: 'Golang Tutorial',
        url: 'https://www.golangprograms.com/',
      },
      {
        name: 'Interactive Vim tutorial',
        url: 'https://openvim.com/',
      },
      {
        name: 'IT Hare on Soft.ware',
        url: 'http://ithare.com/',
      },
      {
        name: 'Web Skills',
        url: 'https://andreasbm.github.io/web-skills/?compact',
      },
      {
        name: 'LFS system',
        url: 'http://www.linuxfromscratch.org/',
      },
      {
        name: 'positronX.io',
        url: 'https://www.positronx.io/',
      },
      {
        name: 'Baeldung',
        url: 'https://www.baeldung.com/',
      },
      {
        name: 'Kubernetes Documentation',
        url: 'https://kubernetes.io/docs/home/',
      },
      {
        name: 'Computer Networks: A Systems Approach',
        url: 'https://book.systemsapproach.org/',
      },
      {
        name: 'Indie Hackers',
        url: 'https://www.indiehackers.com/',
      },
      {
        name: 'iximiuz computer network',
        url: 'https://iximiuz.com/en/posts/computer-networking-101/',
      },
    ],
  },
  {
    category: 'Trending',
    links: [
      {
        name: 'GitHub Trending',
        url: 'https://github.com/trending',
      },
      {
        name: 'Google Trends',
        url: 'https://trends.google.com/trends/trendingsearches/daily?geo=US&hl=en-US',
      },
    ],
  },
  {
    category: 'Reading',
    links: [
      {
        name: '三联生活周刊',
        url: 'https://www.lifeweek.com.cn/',
      },
      {
        name: 'Popular Science',
        url: 'https://www.popsci.com/',
      },
      {
        name: "Reader's Digest",
        url: 'https://www.rd.com/',
      },
      {
        name: 'Quanta Magazine',
        url: 'https://www.quantamagazine.org/',
      },
      {
        name: 'HistoryMaps',
        url: 'https://history-maps.com/',
      },
    ],
  },
  {
    category: 'Technology Industry',
    links: [
      {
        name: 'IETF Datatracker',
        url: 'https://datatracker.ietf.org/',
      },
      {
        name: 'Product Hunt',
        url: 'https://www.producthunt.com/',
      },
      {
        name: 'IT桔子',
        url: 'https://www.itjuzi.com/',
      },
    ],
  },
  {
    category: 'Design',
    links: [
      {
        name: 'Dribbble',
        url: 'https://dribbble.com/',
      },
      {
        name: 'Awwwards',
        url: 'https://www.awwwards.com/',
      },
      {
        name: 'Freepik',
        url: 'https://www.freepik.com/',
      },
      {
        name: 'Color Hunt',
        url: 'https://colorhunt.co/',
      },
      {
        name: '500px',
        url: 'https://500px.com/popular',
      },
      {
        name: 'Unsplash',
        url: 'https://unsplash.com/',
      },
      {
        name: 'Pexels',
        url: 'https://www.pexels.com/',
      },
    ],
  },
  {
    category: 'Calligraphy',
    links: [
      {
        name: '汉典',
        url: 'https://www.zdic.net/hans/%E6%96%87',
      },
      {
        name: '以观书法',
        url: 'https://web.ygsf.com/',
      },
      {
        name: '书法字典-A',
        url: 'https://www.shufadict.com/',
      },
      {
        name: '书法字典-B',
        url: 'http://www.shufazidian.com/s.php',
      },
      {
        name: '云章书法字典',
        url: 'http://shufa.farig.net/',
      },
      {
        name: 'Wengu-300 Tang poems',
        url: 'http://wengu.tartarie.com/wg/wengu.php?l=Tangshi&no=-1',
      },
      {
        name: '临石门颂(Ode to Shimen)',
        url: 'https://patricksiu.org/a-copy-of-ode-to-shimen-%E8%87%A8%E7%9F%B3%E9%96%80%E9%A0%8C/',
      },
    ],
  },
  {
    category: 'Resource',
    links: [
      {
        name: '虫部落•快搜',
        url: 'https://search.chongbuluo.com/',
      },
      {
        name: 'Search Hacker News',
        url: 'https://hn.algolia.com/',
      },
      {
        name: 'Emojipedia',
        url: 'https://emojipedia.org/',
      },
      {
        name: '全栈导航',
        url: 'https://www.itnavs.com/',
      },
      {
        name: 'DogeDoge',
        url: 'https://www.dogedoge.com',
      },
      {
        name: '游戏基因',
        url: 'https://gamegene.cn/',
      },
      {
        name: 'Iconfinder',
        url: 'https://www.iconfinder.com/',
      },
      {
        name: 'FlatIcon',
        url: 'https://www.flaticon.com/',
      },
    ],
  },
  {
    category: 'Application',
    links: [
      {
        name: '少数派',
        url: 'https://sspai.com/',
      },
      {
        name: 'ifanr',
        url: 'https://www.ifanr.com/',
      },
      {
        name: 'Setapp',
        url: 'https://setapp.com/',
      },
      {
        name: 'Mac User',
        url: 'https://learnku.com/mac',
      },
      {
        name: 'MacIdea',
        url: 'http://mac.pcbeta.com/',
      },
      {
        name: '小众软件',
        url: 'https://www.appinn.com/',
      },
    ],
  },
  {
    category: 'Learning',
    links: [
      {
        name: '3Blue1Brown',
        url: 'https://www.3blue1brown.com/',
      },
      {
        name: 'arXiv',
        url: 'https://arxiv.org/',
      },
      {
        name: 'Math ∩ Programming',
        url: 'https://jeremykun.com/primers/',
      },
      {
        name: 'better explained',
        url: 'https://betterexplained.com/cheatsheet/',
      },
      {
        name: 'The Feynman Physics Lectures',
        url: 'https://www.feynmanlectures.caltech.edu/I_toc.html',
      },
      {
        name: 'Explorable Physics',
        url: 'https://landgreen.github.io/physics/index.html',
      },
      {
        name: 'duckduckbang',
        url: 'https://mosermichael.github.io/duckduckbang/html/main.html',
      },
      {
        name: 'GeoGebra 数学应用',
        url: 'https://www.geogebra.org/',
      },
      {
        name: 'Josh W Comeau',
        url: 'https://joshwcomeau.com/',
      },
      {
        name: 'MySQL team',
        url: 'https://mysqlserverteam.com/',
      },
      {
        name: 'mmmEnglish',
        url: 'https://www.mmmenglish.com/',
      },
      {
        name: 'Symbolab',
        url: 'https://zs.symbolab.com/',
      },
      {
        name: 'Etymology Dictionary',
        url: 'https://www.etymonline.com/',
      },
      {
        name: 'Chinese Etymology',
        url: 'https://hanziyuan.net/',
      },
      {
        name: 'Defensive Coding Guide',
        url: 'https://developers.redhat.com/articles/defensive-coding-guide',
      },
      {
        name: 'Seeing Theory',
        url: 'https://seeing-theory.brown.edu/',
      },
    ],
  },
  {
    category: 'Figure & Blog',
    links: [
      {
        name: 'Richard Stallman',
        url: 'https://stallman.org/',
      },
      {
        name: 'Brendan Gregg',
        url: 'https://www.brendangregg.com/',
      },
      {
        name: 'LessWrong',
        url: 'https://www.lesswrong.com/',
      },
      {
        name: 'GatesNotes',
        url: 'https://www.gatesnotes.com/',
      },
      {
        name: 'Benjamin Cane',
        url: 'https://bencane.com/',
      },
      {
        name: 'Go 语言原本',
        url: 'https://golang.design/under-the-hood/',
      },
      {
        name: 'No Headback',
        url: 'https://xargin.com/',
      },
      {
        name: '面向信仰编程',
        url: 'https://draveness.me/',
      },
      {
        name: '煎鱼',
        url: 'https://eddycjy.com/posts/',
      },
      {
        name: 'Coding Horror',
        url: 'https://blog.codinghorror.com/',
      },
      {
        name: '离别歌',
        url: 'https://www.leavesongs.com/',
      },
      {
        name: '鸟窝',
        url: 'https://colobu.com/',
      },
      {
        name: '格物',
        url: 'https://shockerli.net/',
      },
      {
        name: 'Go Web 编程',
        url: 'https://astaxie.gitbooks.io/build-web-application-with-golang/content/zh/',
      },
      {
        name: 'Soham Kamani',
        url: 'https://www.sohamkamani.com/blog',
      },
      {
        name: 'Go 101',
        url: 'https://go101.org/article/101.html',
      },
      {
        name: 'Kelvin Nguyen',
        url: 'https://kelvinnguyen97.com/',
      },
      {
        name: 'Animagraffs',
        url: 'https://animagraffs.com/',
      },
      {
        name: 'Bartosz Ciechanowski',
        url: 'https://ciechanow.ski/',
      },
      {
        name: 'Programiz C programming',
        url: 'https://www.programiz.com/c-programming',
      },
      {
        name: 'Mohammad M. Ramezanpour',
        url: 'https://ramezanpour.net/',
      },
      {
        name: 'stitcher.io',
        url: 'https://stitcher.io/',
      },
      {
        name: 'journaldev',
        url: 'https://www.journaldev.com/',
      },
      {
        name: 'Play with Go',
        url: 'https://play-with-go.dev/',
      },
      {
        name: 'Iouri Khramtsov',
        url: 'https://i-kh.net/',
      },
      {
        name: 'Road 2 Coding',
        url: 'https://www.r2coding.com/',
      },
      {
        name: 'Tech With Maddy',
        url: 'https://techwithmaddy.com/',
      },
      {
        name: '小林coding',
        url: 'https://xiaolincoding.com/',
      },
      {
        name: 'samwho',
        url: 'https://samwho.dev/blog/',
      },
      {
        name: 'Piotr Kołaczkowski',
        url: 'https://pkolaczk.github.io/',
      },
    ],
  },
  {
    category: 'tools',
    links: [
      {
        name: 'asciinema',
        url: 'https://asciinema.org/',
      },
      {
        name: 'open benchmarking - hashcat',
        url: 'https://openbenchmarking.org/test/pts/hashcat',
      },
      {
        name: 'SYSTEMTAP BEGINNERS GUIDE',
        url: 'https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/systemtap_beginners_guide/index',
      },
      {
        name: 'Amp What | special character',
        url: 'https://www.amp-what.com/',
      },
      {
        name: 'Pro Git',
        url: 'https://git-scm.com/book/en/v2',
      },
    ],
  },
  {
    category: 'Car & Trip',
    links: [
      {
        name: '新车评网',
        url: 'https://www.xincheping.com/',
      },
      {
        name: '爱卡汽车论坛',
        url: 'https://club.xcar.com.cn/',
      },
      {
        name: 'Roadtrippers',
        url: 'https://roadtrippers.com/',
      },
      {
        name: 'Car and Driver',
        url: 'https://www.caranddriver.com/',
      },
      {
        name: 'How a Car Works',
        url: 'https://www.howacarworks.com/',
      },
      {
        name: 'How Cars Work',
        url: 'https://auto.howstuffworks.com/car.htm',
      },
      {
        name: 'Car Talk',
        url: 'https://www.cartalk.com/',
      },
    ],
  },
  {
    category: 'exploration',
    links: [
      {
        name: 'Towards Data Science',
        url: 'https://towardsdatascience.com/',
      },
      {
        name: 'Hugging Face',
        url: 'https://huggingface.co/models',
      },
      {
        name: 'Distill',
        url: 'https://distill.pub/',
      },
      {
        name: 'Dive into Deep Learning',
        url: 'https://d2l.ai/',
      },
    ],
  },
  {
    category: 'blockchain & web3',
    links: [
      {
        name: 'Bitcoin',
        url: 'https://bitcoin.org/en/',
      },
      {
        name: 'Go Ethereum',
        url: 'https://geth.ethereum.org/docs/',
      },
      {
        name: 'solidity',
        url: 'https://soliditylang.org/',
      },
      {
        name: 'Market Cap',
        url: 'https://coinmarketcap.com/',
      },
      {
        name: 'AICoin',
        url: 'https://www.aicoin.com/en',
      },
    ],
  },
  {
    category: 'misc.',
    links: [
      {
        name: 'Awesome Remote Job',
        url: 'https://github.com/lukasz-madon/awesome-remote-job',
      },
      {
        name: '电鸭',
        url: 'https://eleduck.com/',
      },
      {
        name: 'React Podcast',
        url: 'https://reactpodcast.com/',
      },
      {
        name: 'NTP Servers in China',
        url: 'https://www.ntppool.org/zone/cn',
      },
      {
        name: 'HTTP Archive',
        url: 'https://httparchive.org/',
      },
    ],
  },
  {
    category: 'personal',
    links: [
      {
        name: 'Slack Gophers',
        url: 'https://app.slack.com/client/T029RQSE6/C029WG6AM',
      },
      {
        name: 'Discord',
        url: 'https://discord.com/channels/@me',
      },
    ],
  },
  {
    category: 'learn sth',
    links: [
      {
        name: 'DPLA(Digital Public Library of America)',
        url: 'https://dp.la/',
      },
      {
        name: 'wikiHow',
        url: 'https://www.wikihow.com/Main-Page',
      },
      {
        name: 'Quora',
        url: 'https://www.quora.com/',
      },
      {
        name: '默沙东诊疗手册',
        url: 'https://www.msdmanuals.cn/home',
      },
      {
        name: 'WPS academy',
        url: 'https://www.wps.com/academy/',
      },
      {
        name: 'Regex Learn',
        url: 'https://regexlearn.com/',
      },
      {
        name: 'Learn X in Y minutes',
        url: 'https://learnxinyminutes.com/docs/python/',
      },
    ],
  },
]

export default links
